...
//Adds Page Content
add_action( 'genesis_before_loop', 'brotherhood_do_portfolio_content' );
function brotherhood_do_portfolio_content() {
 ...
 * @since 1.0.0
 */
add_action( 'genesis_entry_header', 'brotherhood_portfolio_do_post_image' ); 
function brotherhood_portfolio_do_post_image() { 
...
add_action( 'genesis_before_content', 'brotherhood_portfolio_page_widgets' );
function brotherhood_portfolio_page_widgets() {

    echo '<h2 class="screen-reader-text">' . __( 'Main Content', 'brotherhood' ) . '</h2>';

    genesis_widget_area( 'portfolio-page-hero', array(
        'before' => '<div id="portfolio-page-hero" class="portfolio-page-hero"><div class="widget-area"><div class="wrap">',
        'after'  => '</div></div></div>',
    ) );
}

// Add Content for the Portfolio posts in this Page Template
add_action( 'genesis_entry_content', 'brotherhood_portfolio_do_post_content' );
function brotherhood_portfolio_do_post_content() {
    
    if ( genesis_get_option( 'brotherhood_portfolio_content' ) == 'excerpts' ) {
        the_excerpt();
    
    } else {
        if ( genesis_get_option( 'brotherhood_portfolio_content_archive_limit' ) )
            the_content_limit( (int)genesis_get_option( 'brotherhood_portfolio_content_archive_limit' ), __( 'Read More', 'brotherhood' ) );
        else
            the_content(__( 'Read More', 'brotherhood' ));
    }
} 
...
function portfolio_loop() {
    ...
    $include = genesis_get_option( 'brotherhood_portfolio_cat' );
    $exclude = genesis_get_option( 'brotherhood_portfolio_cat_exclude' ) ? explode(',', str_replace(' ', '', genesis_get_option( 'brotherhood_portfolio_cat_exclude' ))) : '';
        
    $cf = genesis_get_custom_field( 'query_args' ); // Easter Egg
    $args = array( 'cat' => $include, 'category__not_in' => $exclude, 'showposts' => genesis_get_option( 'brotherhood_portfolio_cat_num' ), 'paged' => $paged);
